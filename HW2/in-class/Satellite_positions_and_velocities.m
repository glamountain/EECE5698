...
%
% Pau Closas, 2018: initializes variables
...
inclination_angle = 55;  % Inclination angle of satellites (deg)
GNSS_config.const_delta_lambda = 0; % Longitude offset of constellation (deg)
GNSS_config.const_delta_t = 0;      % Timing offset of constellation (s)


% Begins

% Convert inclination angle to degrees
inclination = degtorad(inclination_angle); 

% Determine orbital angular rate using (8.8)
omega_is = sqrt(mu / r_os^3);

% Determine constellation time
const_time = time + GNSS_config.const_delta_t;

sat_r_es_e = zeros(GNSS_config.no_sat,3);
sat_v_es_e = zeros(GNSS_config.no_sat,3);
...
for j = 1:GNSS_config.no_sat
    ...
    r_os_o = r_os*[cos(u_os_o);sin(u_os_o);0];
    ...
    % is omega_is.
    v_os_o = r_os*omega_is*[-sin(u_os_o);cos(u_os_o);0];
